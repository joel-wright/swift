...
This feature builds upon the data segment support within SLO to
...
def _tar_padding(size):
    ...
    return {
        'data': base64.b64encode('\x00' * (_bs - (size - 1) % _bs - 1))
    }
...
def _tar_header(name, size, mode=0o644):
    ...
    return {
        'data': base64.b64encode(info.tobuf(format=tarfile.GNU_FORMAT))
    }


def parse_and_validate_input(req_body):
    """
    ...
    for seg_index, seg_dict in enumerate(parsed_data):
        ...
        seg_dict.update(parsed_seg_data)
        slo_data.append(seg_dict)
...
class TarLargeObject(object):
    ...
    def _process_seg_dict(seg_dict):
        ...
        header = _tar_header(seg_name, seg_size, seg_mode)
        padding = _tar_padding(seg_size)

        return [header, processed, padding]
    ...
    def handle_tar_put(self, req, start_response):
        ...
        for seg_dict in parsed_data:
            processed_data.extend(self._process_seg_dict(seg_dict))
